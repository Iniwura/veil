...
export async function discoverAccounts(
  ...
  const latest = latestBlock ?? (await hardhatEthers.provider.getBlockNumber());
  const logChunkSize = configuredInteger(
    "UNVEIL_V4_KEEPER_LOG_CHUNK_SIZE",
  ...
  const requested = await queryFilterInChunks(
    (chunkFrom, chunkTo) =>
      contracts.seatKeeper.queryFilter(contracts.seatKeeper.filters.SeatAttestationRequested(), chunkFrom, chunkTo),
    fromBlock,
    latest,
    logChunkSize,
  );
  ...
  const renewed = await queryFilterInChunks(
    (chunkFrom, chunkTo) => contracts.pool.queryFilter(contracts.pool.filters.ShardedSeatRenewed(), chunkFrom, chunkTo),
    fromBlock,
    latest,
    logChunkSize,
  );
