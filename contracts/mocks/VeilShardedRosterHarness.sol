...
import {VeilShardedRoster} from "../draw/VeilShardedRoster.sol";

contract VeilShardedRosterHarness is VeilShardedRoster {
    ...
    function acquireMany(address[] calldata accounts) external {
        for (uint256 i = 0; i < accounts.length; i++) _acquireOrRenewShardedSeat(accounts[i]);
    }
    ...
    function pruneShard(uint8 shard) external {
    ...
    function historicalSeat(
        ...
        uint8 shard,
        uint8 index
    ) external view returns (address account, uint64 expiresAt, uint256 eligibleFromRoundId) {
        (account, expiresAt, , eligibleFromRoundId) = _historicalShardPlayerAt(roundId, shard, index);
    }

    function historicalParticipantCount(uint256 roundId, uint8 shard, uint64 closesAt) external view returns (uint8) {
        return _historicalShardParticipantCount(roundId, shard, closesAt);
    }
