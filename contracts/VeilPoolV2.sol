...
// Slice 3A keeps the reviewed bounded FHE draw engine separate from the production custody
// boundary. The legacy VeilPool remains unchanged for regression coverage.
...
interface IVeilStrategyManagerV2 {
...
interface IVeilPoolManagerCallback {
...
/// @title VeilPoolV2
/// @notice Production Slice 3A pool boundary: encrypted user positions and draw weights remain
///         in the pool while confidential principal custody and settlement live in the manager.
/// @dev The draw/schedule/epoch logic intentionally mirrors the reviewed VeilPool implementation.
contract VeilPoolV2 is ZamaEthereumConfig {
    ...
    // This is intentionally public: account ownership is nonfinancial request metadata used by
    // cancelWithdrawal and authenticated manager callbacks. Amounts remain encrypted/private.
    ...
    function configureStrategyManager(address manager_) external {
        ...
        require(IVeilStrategyManagerV2(manager_).pool() == address(this), "Invalid manager pool");
        require(IVeilStrategyManagerV2(manager_).principalAsset() == address(asset), "Invalid manager asset");
    ...
    function deposit(externalEuint64 encryptedAmount, bytes calldata inputProof) external {
        ...
        IVeilStrategyManagerV2(strategyManager).recordPrincipalDeposit(msg.sender, transferred);
    ...
    function withdraw(externalEuint64 encryptedAmount, bytes calldata inputProof) external returns (uint256 requestId) {
        ...
        (requestId, acceptedAmount, , queuedAmount) = IVeilStrategyManagerV2(strategyManager)
    ...
    function cancelWithdrawal(uint256 requestId) external returns (euint64 canceledAmount) {
        ...
        canceledAmount = IVeilStrategyManagerV2(strategyManager).cancelPrincipalWithdrawal(requestId);
    }
